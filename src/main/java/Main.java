...
public final class Main {
  ...
  public static class MyPipeline implements VisionPipeline {
    ...
    public MyPipeline(){
      myStream = new MjpegServer("processedVideo", 9999);
      mySecondStream = new MjpegServer("lessProcessedVideo", 3314);
      source1 = new CvSource("myImage", VideoMode.PixelFormat.kMJPEG,640,480,30);
      source2 = new CvSource("mySecondImage", VideoMode.PixelFormat.kMJPEG,640,480,30);
      myStream.setSource(source1);
      mySecondStream.setSource(source2);

      nt = NetworkTableInstance.getDefault();
      table = nt.getTable("Ball Target");
    }
    ...
    public void process(Mat mat) {
      ...
      Mat secondHSV = new Mat();

      Imgproc.cvtColor(mat, hsv, Imgproc.COLOR_BGR2HSV);                                            //Converts color image in BGR to HSV
      Core.inRange(hsv, new Scalar(17.805755395683452, 32.10431654676259,100.0),       
      new Scalar(43.31058020477816, 255.0, 255.0), inRangeHSV);                                 
      Imgproc.line(mat, pt1, pt2, new Scalar(0,0,255),5);
      //Imgproc.blur(inRangeHSV, blurred, new Size(36, 36));
      Imgproc.erode(inRangeHSV, eroded, kernel);
      Imgproc.blur(eroded, blurred, new Size(36, 36));
      Core.inRange(blurred, new Scalar(128), new Scalar(255), secondHSV);
      Imgproc.findContours(secondHSV, contours, hierarchy, Imgproc.RETR_TREE, Imgproc.CHAIN_APPROX_SIMPLE);
